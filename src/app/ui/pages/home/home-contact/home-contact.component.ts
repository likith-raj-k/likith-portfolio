...
export class HomeContactComponent implements OnInit {
  ...
  _onSubmit(): void {
    ...
    console.log(this._mFormGroup.value); // Log the plain form data
  
    emailjs
      .send(
        environment.emailJs.serviceId,
        environment.emailJs.templateId,
        { ...this._mFormGroup.value }, // Use the plain form values
        { publicKey: environment.emailJs.publicKey }
      )
      .then(() => {
        console.log('Email sent successfully');
        this.closeProgress(); // Close the progress dialog
        this.openSuccess();   // Show the success dialog
        this._mFormGroup.reset(); // Reset the form
      })
      .catch((error) => {
        console.error('Error sending email:', error);
        this.closeProgress(); // Ensure the progress dialog is closed
